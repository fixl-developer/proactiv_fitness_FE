        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => { setIsLoading(true); loadFlags() }}>
                    ...
                    <Card className="border-blue-200 shadow-lg">
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="ghost" size="sm" onClick={() => { setShowModal(false); setEditingFlag(null) }}><X className="w-4 h-4" /></Button>
                            </div>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="My Feature Flag" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" />
                                </div>
                            </div>
                            <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <select value={form.environment} onChange={e => setForm(f => ({ ...f, environment: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none">
                                ...
                                <div>
                                    ...
                                    <select value={form.enabled ? 'enabled' : 'disabled'} onChange={e => setForm(f => ({ ...f, enabled: e.target.value === 'enabled', rolloutPercentage: e.target.value === 'enabled' ? 25 : 0 }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none">
                                ...
                                <div>
                                    ...
                                    <input type="number" min="0" max="100" value={form.rolloutPercentage} onChange={e => setForm(f => ({ ...f, rolloutPercentage: Math.min(100, Math.max(0, parseInt(e.target.value) || 0)) }))} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none" />
                                </div>
                            </div>
                            <div className="flex justify-end gap-2">
                                <Button variant="outline" size="sm" onClick={() => { setShowModal(false); setEditingFlag(null) }}>Cancel</Button>
            ...
            <Card>
                <CardContent className="p-4">
                    <div className="relative">
                        ...
                        <input type="text" placeholder="Search feature flags..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all" />
                    </div>
                </CardContent>
            </Card>
                                    ...
                                    <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                                        ...
                                        <div className="flex items-center gap-6 lg:gap-8">
                                            <div className="w-48">
                                                ...
                                                <input type="range" min="0" max="100" step="5" value={flag.rolloutPercentage} onChange={(e) => updateRollout(flag, Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600" />
                                                <Progress value={flag.rolloutPercentage} className="h-1.5 mt-1" />
                                            </div>

                                            <button onClick={() => toggleFlag(flag)} className="flex items-center gap-2 shrink-0">
                                                {flag.enabled ? <ToggleRight className="w-10 h-10 text-green-500" /> : <ToggleLeft className="w-10 h-10 text-gray-400" />}
                                            </button>

                                            <Button variant="ghost" size="sm" onClick={() => openEditModal(flag)} className="text-blue-500 hover:text-blue-700 shrink-0">
                                                <Edit2 className="w-4 h-4" />
                                            </Button>

                                            {deleteConfirm === flag.id ? (
                                                <div className="flex items-center gap-1 shrink-0">
                                                    <Button variant="ghost" size="sm" onClick={() => handleDelete(flag)} className="text-red-600 hover:text-red-800 text-xs">Confirm</Button>
                                                    <Button variant="ghost" size="sm" onClick={() => setDeleteConfirm(null)} className="text-gray-500 text-xs">Cancel</Button>
                                                </div>
                                            ) : (
                                                <Button variant="ghost" size="sm" onClick={() => setDeleteConfirm(flag.id)} className="text-red-500 hover:text-red-700 shrink-0">
